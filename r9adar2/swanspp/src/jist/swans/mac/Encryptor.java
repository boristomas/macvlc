...
public class Encryptor {
	...
	private static String keyStr ="RTcCQar3pUikoGVI";//128bits
	...
	synchronized public static byte[] encrypt(Message msg) throws Exception {
		init();
		fraction = (double)msg.getSize()/(double)Constants.UDP_MAX_SIZE;
		delay = (long)(Constants.MICRO_SECOND * 190 * fraction);

		JistAPI.sleep(delay);//http://www.cse.wustl.edu/~jain/cse567-06/ftp/encryption_perf/
	...
	synchronized public static Message decrypt(byte[] array) throws Exception {
		init();
		fraction = (double)array.length/(double)Constants.UDP_MAX_SIZE;
		delay = (long)(Constants.MICRO_SECOND * 120 * fraction);
